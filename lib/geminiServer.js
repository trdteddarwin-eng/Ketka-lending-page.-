...
// Hard caps so a hostile caller can't pump giant prompts (input tokens) or
// pull unbounded output through the chat endpoint. Sophie's replies are short.
...
export async function generateChatReply(apiKey, history, message) {
  ...
  turns.forEach((msg) => {
    ...
    const text = msg && typeof msg.text === 'string' ? msg.text.slice(0, MAX_TURN_CHARS) : '';
    prompt += `${who}: ${text}\n`;
  });
  prompt += `Customer: ${String(message || '').slice(0, MAX_TURN_CHARS)}\nSophie:`;
